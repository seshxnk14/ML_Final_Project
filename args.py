...
# Mod 2 / HuBERT eval extras (ignored by scripts that do not use them)
parser.add_argument(
    "--readout",
    type=str,
    default="both",
    choices=["ce", "attention", "both"],
    help="Query decision from CE head, attention head, or both (AND).",
)
parser.add_argument(
    "--decision_threshold",
    type=float,
    default=0.5,
    help="Sigmoid threshold for attention path.",
)
parser.add_argument(
    "--min_event_sec",
    type=float,
    default=0.0,
    help="Suppress predicted positive runs shorter than this many seconds (0=off).",
)
